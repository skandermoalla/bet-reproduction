...
def compute_blockpush_metrics(
    ...
    for rollout_i, rollout in enumerate(data):
        ...
        # By default, if the episode is not finished, it is set to 0 in done_array
        if eval_was_vectorized:
            if done_at_array[rollout_i] == 0:
                done_at_array[rollout_i] = data.shape[1] - 1
...
def compute_sequence_entropy(task_mappings):
    """
    Compute the sequence entropy from a list of task mappings.
    Args:
...
def compute_task_entropy(observations_dict):
    """
    Compute the task entropy of a list of task mappings.
    This function is directly taken from the author's code and slightly modified to avoid sub-sampling and averaging out the entropy.
    Args:
        observations_dict (dict): dictionary with the sequences and the number of times they appear
    Returns:
        entropy (float): task entropy
    """

    elements, counts = observations_dict.keys(), observations_dict.values()

    task_count = {}
    total_tasks = 0
    for tasks in elements:
        for task in tasks:
            task_count[task] = task_count.get(task, 0) + 1
            total_tasks += 1
        if len(tasks) < 5:
            task_count["."] = task_count.get(".", 0) + 5 - len(tasks)
            total_tasks += 5 - len(tasks)
    entropy = sum(
        -(x / total_tasks) * np.log2((x / total_tasks)) for x in task_count.values()
    )

    return entropy


def compute_sequence_dict(sequences):
    """
    Compute how many times each sequence appears in the dataset.
    Args:
        sequences: list of lists of strings, each list of strings is a sequence of tasks.
    Returns:
        sequence_dict: dictionary, where the keys are the sequences and the values are the number of times each sequence appears.
    """
    sequence_dict = {}
    for sequence in sequences:
        if tuple(sequence) in sequence_dict:
            sequence_dict[tuple(sequence)] += 1
        else:
            sequence_dict[tuple(sequence)] = 1

    # Sort the dictionary by the number of times each sequence appears
    sequence_dict = dict(
        sorted(sequence_dict.items(), key=lambda item: item[1], reverse=True)
    )

    return sequence_dict


def get_values_counts(d):
    """
    Get the values and counts of a dictionary.
    Args:
        d (dict): dictionary
    Returns:
        elements (list): list of values
        counts (list): list of counts
    """
    elements, counts = d.keys(), d.values()

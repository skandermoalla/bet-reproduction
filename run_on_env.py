...
def run_evaluations(cfg):
    """ Run evaluations for all snapshots in the load_dir.
    Args:
...
def process_cv_run_idxs(cfg):
    """ Process the cv_run_idx parameter in the config file.
    Args:
        cfg (DictConfig): Config file.
    Returns:
        list: List of cv_run_idx to evaluate.
    """
...
def run_snapshot(cfg):
    """ Run evaluations for a single snapshot.
    Args:
        cfg (DictConfig): Config file.
    """

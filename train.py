...
class Workspace:
    ...
    def run(self):
        ...
        if self.cfg.experiment.save_latents:
            self.save_latents()
        if self.cfg.experiment.log_dataset:
            wandb.log({"training_dataset": wandb.Image("training_dataset.png")})
        if self.cfg.experiment.log_bins:
            self.log_bins()
        if self.cfg.experiment.log_train_bin_assignments:
            self.log_train_bin_assignments()
        ...
        for epoch in self.state_prior_iterator:
            ...
            if ((self.prior_epoch + 1) % self.cfg.experiment.save_prior_every) == 0:
                #   # Last checkpoint.
                last_path = Path(self.work_dir, "last")
                last_path.mkdir(exist_ok=True)
                filename = last_path / f"snapshot_{self.cfg.experiment.cv_run_idx}.pt"
                payload = {k: self.__dict__[k] for k in self._keys_to_save}
                torch.save(payload, filename)

                if self.current_test_loss < self.best_model_test_loss:
                    self.best_model_test_loss = self.current_test_loss
                    log.info(
    ...
    def log_bins(self):
        bins = self.action_ae.bin_centers
        wandb.log(
            {
                "action_bins": wandb.Table(
                    columns=[f"D{i}" for i in range(bins.shape[1])],
                    data=bins.cpu().numpy(),
                )
            }
        )

    def log_train_bin_assignments(self):
        """Has limited support."""
        actions = torch.unique(self.train_loader.dataset._get_all_actions(), dim=0).to(
            self.device
        )
        enc_actions = self.action_ae.encode_into_latent(actions)
        if type(enc_actions) == tuple:
            assignments = enc_actions[0]
        else:
            assignments = enc_actions
        wandb.log(
            {
                "action_bin_assignments": wandb.Table(
                    columns=[f"D{i}" for i in range(actions.shape[1])]
                    + ["bin_assignment"],
                    data=torch.cat([actions.cpu(), assignments.cpu()], dim=1).numpy(),
                )
            }
        )

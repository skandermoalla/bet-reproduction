...
for job in {0..7}; do
  python compute_metrics.py \
    load_dir="$(pwd)"/eval_runs/eval_blockpush/reproduction/sweep_num_bins/"$job" \
    tags=\'num_bins,sweep,compute_metrics\' \
    save_subdir=blockpush/reproduction/sweep_num_bins/"$job"
done